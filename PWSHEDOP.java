...
public class PWSHEDOP<T extends IntegerType<T>, L extends Comparable<L>> extends AbstractOp {
        ...
        public void run() {
                ...
                /*
                 * Create a "Pixel" for each pixel in the input
        ...
        private void RandomWalker(ArrayList<PseudoEdge<T, L>> edgesLCP, ArrayList<Pixel<T, L>> pixelsLCP) {
                ...
                //A(i,i)=n means there are n edges on this plateau that are incident to node i
                int rnzs = 0;
                int rnzu = 0;
                for (Pixel<T, L> p : pixelsLCP) {
                        if (local_seeds.contains(p)) {
                ...
                //A(i,j)=n means that node ith and jth unseeded nodes are connected by -n-1 edges 
                //B(i,j)=n means that the ith unseeded and the jth seeded node are connected by -n-1 edges
